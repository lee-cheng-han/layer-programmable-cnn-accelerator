...
module tb_programmable_system_top;
  ...
  localparam logic [11:0] ADDR_PERF_JOB_CYCLES = 12'h080;
  localparam logic [11:0] ADDR_PERF_CONTROLLER_CYCLES = 12'h084;
  localparam logic [11:0] ADDR_PERF_COMPUTE_CYCLES = 12'h088;
  localparam logic [11:0] ADDR_PERF_PARAMETER_STALLS = 12'h08C;
  localparam logic [11:0] ADDR_PERF_INPUT_STARVATION = 12'h090;
  localparam logic [11:0] ADDR_PERF_OUTPUT_BACKPRESSURE = 12'h094;
  localparam logic [11:0] ADDR_PERF_INPUT_BYTES = 12'h098;
  localparam logic [11:0] ADDR_PERF_OUTPUT_BYTES = 12'h09C;
  localparam logic [11:0] ADDR_PERF_COUNTING = 12'h0AC;
  localparam logic [11:0] ADDR_PERF_ISSUED_MACS_LO = 12'h0B0;
  localparam logic [11:0] ADDR_PERF_ISSUED_MACS_HI = 12'h0B4;
  localparam logic [11:0] ADDR_PERF_LAYER0_CYCLES = 12'h0C0;
  ...
  initial begin
    ...
    axi_read(ADDR_PERF_JOB_CYCLES, value);
    if (value == 0) $fatal(1, "job cycle counter is empty");
    axi_read(ADDR_PERF_CONTROLLER_CYCLES, value);
    if (value == 0) $fatal(1, "controller cycle counter is empty");
    axi_read(ADDR_PERF_COMPUTE_CYCLES, value);
    if (value == 0) $fatal(1, "compute cycle counter is empty");
    axi_read(ADDR_PERF_PARAMETER_STALLS, value);
    if (value != 0) $fatal(1, "unexpected parameter stalls: %0d", value);
    axi_read(ADDR_PERF_INPUT_STARVATION, value);
    if (value == 0) $fatal(1, "input starvation was not observed");
    axi_read(ADDR_PERF_OUTPUT_BACKPRESSURE, value);
    if (value == 0) $fatal(1, "output backpressure was not observed");
    axi_read(ADDR_PERF_INPUT_BYTES, value);
    if (value != 70) $fatal(1, "input byte count mismatch: %0d", value);
    axi_read(ADDR_PERF_OUTPUT_BYTES, value);
    if (value != 70) $fatal(1, "output byte count mismatch: %0d", value);
    axi_read(ADDR_PERF_COUNTING, value);
    if (value != 0) $fatal(1, "performance snapshot still counting");
    axi_read(ADDR_PERF_ISSUED_MACS_LO, value);
    if (value != 6) $fatal(1, "issued MAC count mismatch: %0d", value);
    axi_read(ADDR_PERF_ISSUED_MACS_HI, value);
    if (value != 0) $fatal(1, "issued MAC high word is nonzero: %0d", value);
    axi_read(ADDR_PERF_LAYER0_CYCLES, value);
    if (value == 0) $fatal(1, "layer 0 cycle counter is empty");
