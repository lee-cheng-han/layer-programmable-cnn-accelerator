...
| programmable AXI-Lite system | High pre-board | AXI-Lite metadata/lifecycle/launch and progress readback, exact stream-byte and cycle/stall snapshots, per-layer timing, sticky structured first-fault capture/clear, malformed parameter rejection, active-model-preserving recovery, and deterministic randomized output backpressure |
